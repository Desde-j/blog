<?php 
    ...
    if(!empty($_POST['message'])){
      ...
      }else{ header('Location:article.php?reg_err=short'); die();}
    }
                  ...
                  if ($resultat){ // Si on a le résultat alors on va comprésser l'image
                    ...
                    if (is_readable("../articles/" .$nom . "." . $extensionUpload)) {
                      ...
                      if($verif_ext['mime'] == $ListeExtension[$extensionUpload]  || $verif_ext['mime'] == $ListeExtensionIE[$extensionUpload]){              
                        ...
                        exit;
                      }else{header('Location:article.php?reg_err=MIME');die();} 
                    }else{header('Location:article.php?reg_err=importerr');die();}
                  }else{header('Location:article.php?reg_err=badformat');die();}
                }else{header('Location:article.php?reg_err=5mo');die();}
              }else{header('Location:article.php?reg_err=dimension');die();} 
            } 
          }
        }
      }   
      header('Location:article.php?reg_err=MIME');die();                  
